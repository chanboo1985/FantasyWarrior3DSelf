...
HPCounter = class("HPCounter",function() return cc.Node:create() end)

function HPCounter:ctor()
...
function HPCounter:showBloodLossNum(dmage,raceType,atack)
	...
	local function getRandomXYZ()
		local random_x = 20*math.sin(math.rad(time*0.5+4356))
        local random_y = 20*math.sin(math.rad(time*0.37+5436))
        local random_z = 20*math.sin(math.rad(time*0.2+54325))
	...
	local function getBlood()
        ...
        if raceType.___racetype == EnumRaceType.MONSTER then
            blood:setColor(cc.c3b(0,180,255))
        end
        
        local function getAction()
        	local sequence = cc.Sequence:create(
        	   cc.EaseElasticOut:create(cc.ScaleTo:create(tm/2,targetScale), 0.4),
